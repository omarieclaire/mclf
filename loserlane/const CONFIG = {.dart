...
class CollisionManager {
   ...
   * @param {SpatialManager} spatialManager - Reference to game's spatial management system
   */
  ...
  /**
   * Performs basic AABB (Axis-Aligned Bounding Box) collision detection
   * Returns true if two rectangular hitboxes overlap in 2D space
   * Uses early-out conditions for optimization
   * @returns {boolean} True if hitboxes intersect
   */
  checkCollision(hitboxA, hitboxB) {
    return !(
      (
        hitboxA.x + hitboxA.width <= hitboxB.x || // A is completely to left of B
        hitboxA.x >= hitboxB.x + hitboxB.width || // A is completely to right of B
        hitboxA.y + hitboxA.height <= hitboxB.y || // A is completely above B
  ...
  /**
   * Specialized collision detection for the player's bike
   * Handles three types of collisions:
   * 1. Moving obstacles (TTCs, vehicles)
   * 2. Parked vehicles and their opening doors
   * 3. Track collisions with special jumping immunity
   * @returns {string|null} Type of collision or null if no collision
   */
  checkBikeCollisionIsSpecial(bikeHitbox, darlings, isJumping) {
    // Check vehicle collisions first - these should always kill
        ...
        if (collisionDirection === "up") {
          switch (darling.type) {
            ...
            case DarlingType.ONCOMING_DEATHMACHINE:
              console.log("hit ONCOMING_DEATHMACHINE");

              return "ONCOMING_DEATHMACHINE";

            case DarlingType.WANDERER:
              console.log("hit WANDERER");
    ...
    // const currentPosition = Math.floor(bikeCenter);

    // // Track collisions respect immunity
    // if (!isJumping && trackPositions.some((track) => track === currentPosition)) {
  ...
  /**
   * Main update loop for collision system
   * Processes all potential collisions and triggers appropriate responses
   * Handles bike collisions separately from other entity collisions
   */
  collisionManagerUpdate() {
    ...
    for (const [entityA, entityB] of pairs) {
      ...
      if (entityA.type === DarlingType.BIKE || entityB.type === DarlingType.BIKE) {
        ...
        // Handle regular entity-entity collisions
  ...
  /**
   * Handles collision response between two non-player entities
   * Uses priority system to determine which entity adjusts its behavior
   */
  handleEntityCollision(entityA, entityB) {
    // Skip if either entity ignores collisions
    if (entityA.behavior.ignoreCollisions || entityB.behavior.ignoreCollisions) {
      return;
    }

    // Get collision priorities to determine response
  ...
  /**
   * Applies physics-based collision response to an entity
   * Handles speed matching, stopping, and temporary pauses
   */
  ...
  /**
   * Returns collision priority for entity types
   * Higher priority entities force lower priority ones to adjust
   */
  getEntityPriority(entity) {
    const priorities = {
      [DarlingType.TTC]: 5, // Highest priority - streetcars
      [DarlingType.TTC_LANE_DEATHMACHINE]: 4, // Vehicles in TTC lanes
      [DarlingType.ONCOMING_DEATHMACHINE]: 3, // Oncoming traffic
      [DarlingType.PARKED_DEATHMACHINE]: 2, // Parked vehicles
      [DarlingType.WANDERER]: 1, // Pedestrians/cyclists
      [DarlingType.BUILDING]: 0, // Static obstacles
    };
  ...
  /**
   * Generates pairs of entities that might collide
   * Uses spatial partitioning for efficiency
   * @returns {Array} Array of [entityA, entityB] pairs
   */
  getCollisionPairs() {
    const pairs = [];
    ...
    for (let i = 0; i < darlings.length; i++) {
      ...
      for (const entityB of nearby) {
        ...
        // Create unique key for entity pair
        const pairKey = [entityA.id, entityB.id].sort().join(",");
        if (processedPairs.has(pairKey)) continue;

        // Check if pair should be tested and is colliding
  ...
  /**
   * Determines if two entities should be checked for collision
   * Optimizes by checking lane proximity and entity types
   */
  shouldCheckCollision(entityA, entityB) {
    ...
    // Special wider check for TTCs
    if (entityA.type === DarlingType.TTC || entityB.type === DarlingType.TTC) {
      return xDistance <= 2; // Check up to 2 lanes away for TTCs
    }

    // Only check adjacent or same lane for other entities
    return xDistance <= 1;
  }

  /**
   * Validates if an entity can move to a new position
   * Prevents movement that would cause invalid collisions
  ...
  validateMovement(entity, newPosition) {
    ...
    // Check nearby entities for collisions
  ...
  /**
   * Calculates direction of collision between two hitboxes
   * Uses center points to determine dominant axis
  ...
  getCollisionDirection(hitboxA, hitboxB) {
    ...
    // Return dominant axis direction
...
class LoserLane {
  ...
  /**
   * Initializes core game components in correct dependency order
   * @private
   */
...
initializeparkedDeathMachines() {
  ...
  handleJump(direction) {
    // if (this.state.isJumping || this.state.isDead) return;
    // if (this.state.currentLane === CONFIG.KILLERLANES.KILLERTRACK1) {
    //   this.state.currentLane = direction === "left" ? CONFIG.KILLERLANES.KILLERTRACK1 - 1 : CONFIG.KILLERLANES.KILLERTRACK1 + 1;
    // } else if (this.state.currentLane === CONFIG.KILLERLANES.KILLERTRACK2) {
    //   this.state.currentLane = direction === "left" ? CONFIG.KILLERLANES.KILLERTRACK2 - 1 : CONFIG.KILLERLANES.KILLERTRACK2 + 1;
    // }
    // this.updateBikePosition();
  }
  ...
  /**
   * Creates and configures a new bike entity with proper positioning and properties.
   * Used when initializing or resetting the bike's state in the game.
   * @returns {BaseEntity} A fully configured bike entity ready for game use
   */
  updateBike() {
    // Initialize new bike entity with proper positioning
    // BaseEntity constructor takes (config, properties, entityType)
    const bikeEntity = new BaseEntity(
      this.config,
      {
        // Calculate center position based on current lane
        // Subtract half width to ensure bike is centered on lane
        // Rather than having left edge aligned with lane
        position: new Position(
          this.state.currentLane - DARLINGS.BIKE.width / 2,
          // Vertical position is fixed at predetermined cyclist height
          CONFIG.GAME.CYCLIST_Y
        ),
      },
      // Specify this is a bike-type entity for type checking and collision handling
      DarlingType.BIKE
    );

    // Set physical dimensions from predefined constants
    bikeEntity.width = DARLINGS.BIKE.width;
    bikeEntity.height = DARLINGS.BIKE.height;

    // Set visual properties for rendering
    bikeEntity.art = DARLINGS.BIKE.art; // Visual representation/sprite
    bikeEntity.color = STYLES.BIKE; // Color scheme/styling

    // Attach bike-specific behavior controller
    // Handles input response, movement patterns, and bike-specific logic
  ...
  /**
   * Checks for and handles collisions between the bike and other game entities.
   * Runs every game tick to ensure immediate collision response.
   * Handles both regular obstacles and special "death machine" collisions.
   */
  checkBikeCollisions() {
    // Skip collision detection if bike is already in death state
    // Prevents multiple death triggers and unnecessary processing
    if (this.state.isDead) return;

    // Define bike's current collision boundary box
    const bikeHitbox = {
      // X position represents bike's center point on lane
      x: this.state.currentLane,
      // Adjust Y position upward when jumping
      // -1 unit elevation during jump for proper collision detection
    ...
    // Sort game entities into collision categories
    const darlingsForCollision = {
      // Regular obstacles -  except bike and parked death machines
      // Filter creates new array excluding specified types
      ...
      // Parked death machines get special collision handling
      // Separate array for death machine specific logic
      parkedDeathMachines: Array.from(this.spatialManager.darlings).filter((e) => e.type === DarlingType.PARKED_DEATHMACHINE),
    };

    

    // Check for collisions considering jump state and obstacle types
    // Returns collision type if hit detected, null/undefined if no collision
    ...
    // Detailed debug logging for collision detection
    // Helps track game state and collision behavior
    console.log("Collision check result:", {
      collisionType: collision,
      currentLane: this.state.currentLane,
      doubleJumpPending: this.doubleJumpPendingGiveMeImmunity,
      // Check if bike is on a killer track (special danger zones)
      onTrack: [CONFIG.KILLERLANES.KILLERTRACK1, CONFIG.KILLERLANES.KILLERTRACK2].includes(Math.floor(this.state.currentLane)),
    });

    // If collision detected, trigger appropriate death sequence
    // Passes collision type to determine death animation/effects
    if (collision) {
      this.die(collision);
    }
  }
